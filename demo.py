...
def clone_speaker(upload_file, clone_speaker_name, cloned_speaker_names):
    ...
    with open(
        os.path.join(OUTPUT, "cloned_speakers", clone_speaker_name + ".json"), "w"
    ) as fp:
    ...
    return (
        upload_file,
        clone_speaker_name,
        cloned_speaker_names,
        gr.Dropdown.update(choices=cloned_speaker_names),
    )


def tts(text, speaker_type, speaker_name_studio, speaker_name_custom, lang):
    embeddings = (
        STUDIO_SPEAKERS[speaker_name_studio]
        if speaker_type == "Studio"
        else cloned_speakers[speaker_name_custom]
    )
...
with gr.Blocks() as demo:
        ...
        with gr.Column() as row4:
            with gr.Row() as col4:
                speaker_name_studio = gr.Dropdown(
                    ...
                    value=(
                        "Asya Anara" if "Asya Anara" in STUDIO_SPEAKERS.keys() else None
                    ),
                )
                speaker_name_custom = gr.Dropdown(
                    ...
                    value=(
                        cloned_speaker_names.value[0]
                        if len(cloned_speaker_names.value) != 0
                        else None
                    ),
                )
            speaker_type = gr.Dropdown(
                label="Speaker type", choices=["Studio", "Cloned"], value="Studio"
            )
        with gr.Column() as col2:
            lang = gr.Dropdown(label="Language", choices=LANUGAGES, value="en")
            text = gr.Textbox(
                label="text", value="A quick brown fox jumps over the lazy dog."
            )
    ...
    with gr.Tab("Clone a new speaker"):
        with gr.Column() as col1:
            ...
            clone_speaker_name = gr.Textbox(
                label="Speaker name", value="default_speaker"
            )
